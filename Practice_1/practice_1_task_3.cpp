...
int main(){
    ...
    cout << "sum: " << sum << endl;
    double avr = (double)sum / N; // Скинул на проверку чату гпт. Он меня поругал что нужно облачить в double для точности, потому что обычно среднее значение - это дробное число. Поэтому поменял. До этого я просто sum = sum / N делал.
